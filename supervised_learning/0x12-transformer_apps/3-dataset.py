...
class Dataset:
    ...
    def __init__(self, batch_size, max_len):
        ...
        """
        self.data_train, info = tfds.load(name='ted_hrlr_translate/pt_to_en',
                                        split='train',
                                        as_supervised=True,
                                        with_info=True)
        ...
        ds_t = (self.data_train
                .filter(lambda x, y: tf.logical_and(tf.size(x) <= max_len,
                                                    tf.size(y) <= max_len))
                .cache()
                .shuffle(buffer_size=num_train_exs)
                .padded_batch(batch_size,
                            padded_shapes=([None], [None]))
                .prefetch(tf.data.AUTOTUNE))
        self.data_train = ds_t
        
        ds_v = (self.data_valid
                .filter(lambda x, y: tf.logical_and(tf.size(x) <= max_len,
                                                    tf.size(y) <= max_len))
                .padded_batch(batch_size,
                            padded_shapes=([None], [None])))
        self.data_valid = ds_v
